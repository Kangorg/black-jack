...
class Table:
    ...
    def add_win(self, start_value, hit_or_stand):
        table_val = self.__table[start_value]
        val_record = table_val[hit_or_stand]
        val_record[0] += 1

    def add_loss(self, start_value, hit_or_stand):
        table_val = self.__table[start_value]
        val_record = table_val[hit_or_stand]
        val_record[1] += 1

    def add_tie(self, start_value, hit_or_stand):
        table_val = self.__table[start_value]
        val_record = table_val[hit_or_stand]
        val_record[2] += 1
    ...
    def print_table(self):
        for values in self.__table:
            print(f'{values}: {self.__table[values]}')
...
while play_game < 100000:
    ...
    dealt_hand = player_hand_value[0]
    if dealt_hand > 21:
        continue
    dealer_hand_value = jake.get_dealer_hand_value()
    random_value = random.randint(1,2)
    if random_value == 2:
        hit_or_stand = 'hitting'
    else:
        hit_or_stand = 'standing'
    if hit_or_stand == 'hitting':
        the_card = the_shoe.get_card_shoe()
        tim.hit(the_card)
        if tim.is_over_21():
            the_table.add_loss(str(dealt_hand), hit_or_stand)
            game_over = True
    if game_over != True:
        while ((dealer_hand_value[0] <= 16) and (dealer_hand_value[1] <= 16)):
            ...
            if jake.is_dealer_over_21():
                the_table.add_win(str(dealt_hand), hit_or_stand)
                game_over = True
                break
        if game_over != True:
            if jake.is_dealer_hand_greater(tim.get_hand_value()):
                ...
                the_table.add_tie(str(dealt_hand), hit_or_stand)
    play_game += 1

#    WIN/LOSS/DRAW
the_table.print_table()
